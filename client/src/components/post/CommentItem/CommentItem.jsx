...
const CommentItem = ({ comment }) => {
  return (
    <article className="hover:bg-gray-50 px-1 sm:px-4">
      <div className="flex pb-4 pt-2 px-2 border-b border-gray-300 sm:px-4">
        <div className="flex-grow-0 flex-shrink-0 pr-2 sm:pr-4">
          ...
          >
            <img loading="lazy" alt={comment.user.profileImage.alt} src={getProfileImagePath(comment.user.profileImage.id)} />
          </Link>
        </div>
        <div className="flex-grow flex-shrink min-w-0">
          <p className="whitespace-nowrap overflow-ellipsis text-xs overflow-hidden">
